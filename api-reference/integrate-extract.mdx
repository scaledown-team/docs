...
- Request body (JSON):
    {
      ...
      "entities": {
        "<TypeName>": "<description string>"
          ...
          | {                                             // classification key
              "labels": [ "<label>" | { "name": "<label>", "rubric": "<when it applies>" }, ... ],  // 2–26 labels
              "description": "<optional context for the decision>"  // optional
            }
      },
  ...
  A key whose object contains a "labels" list is a classification key: it is routed to
  the classification model instead of extracted, and its "confidence" is the calibrated
  probability of the chosen label (returned as the entity "text"). Its "start"/"end"/
  "context" point at the verbatim evidence phrase backing the decision (not the label
  text, which is usually inferred and not present verbatim), falling back to 0 / "" when
  no evidence is found. A classification key with fewer than 2 or more than 26 labels
  returns 422.
- Success response (JSON):
    {
      "entities": [
